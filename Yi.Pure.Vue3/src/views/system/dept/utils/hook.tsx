...
export function useDept() {
  ...
  async function onSearch() {
    loading.value = true;
    const data = (await getDeptList()).data.items; // 这里是返回一维数组结构，前端自行处理成树结构，返回格式要求：唯一id加父节点parentId，parentId取父节点id
    let newData = data;
    if (!isAllEmpty(form.deptName)) {
      // 前端搜索部门名称
      newData = newData.filter(item => item.name.includes(form.deptName));
    }
    if (!isAllEmpty(form.state)) {
      // 前端搜索状态
      newData = newData.filter(item => item.state === form.state);
    }
    ...
    addDialog({
      ...
      props: {
        formInline: {
          ...
          parentId: row?.parentId ?? "00000000-0000-0000-0000-000000000000",
